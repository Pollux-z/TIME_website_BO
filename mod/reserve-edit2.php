                ...
                <div class="form-group row row-sm">
                    ...
                    <div class="col-sm-9">
                        <div class='input-group input-daterange' data-plugin="datepicker" data-option="{todayBtn: 'linked'}">
                            <input type='text' class="form-control" name="from_date" value="<?php echo $from_date;?>" required>
                    ...
                    <div class='input-group input-daterange'>
                        <select name="from_time" id="" class="form-control" required>
                            ...
                            <?php
                                ...
                                $rs = mysqli_query($conn,"SELECT * FROM `db_reserve` WHERE `code` LIKE 'meetingroom' and start_date like '2021-02-22%' ORDER BY `db_reserve`.`start_date` ASC");
                                while($row=mysqli_fetch_assoc($rs)){
                                    $ranges[] = [strtotime($row['start_date']),strtotime($row['end_date'])];
                                }

                                foreach($times as $v){
                                    $chk = strtotime("2021-02-22 $v");
                                    foreach($ranges as $w){
                                        if($chk>=$w[0]&&$chk<$w[1]){
                                            $dab = ' disabled';
                                        }
