...
<?php if(in_array('course',$edits)||$_SESSION['ses_ulevel']>7){?>
    ...
    <a href="?mod=course">My Courses</a> | 
    <a href="?mod=course&page=summary">Summary</a>
</div>
<?php }?>

<?php if($page=='summary'){
    $rss = mysqli_query($conn,"SELECT DISTINCT cid,uid FROM `db_courserec` WHERE end_date is not null");
    ...
    <table id="datatable" class="table table-theme table-row v-middle" data-plugin="dataTable">
        <thead>
            <tr>
                ...
                <th><span class="text-muted">Course Name</span></th>
        ...
        <tbody>
                ...
                while($row=mysqli_fetch_assoc($rs)){       
                    ...
                    echo '<tr class=" " data-id="'.$row['id'].'">
                        ...
                        echo '</div></td><td><a href="?mod=course-edit&id='.$row['id'].'" class="btn btn-sm btn-danger">Edit</a></td>
                        </tr>';
                }
            ?>                                        
...
<?php }else{
    ...
    $rss = mysqli_query($conn,"SELECT a.uid,a.start_date,a.end_date,b.owner FROM `db_courserec` a join db_course b where a.cid = b.id and a.end_date >= '$now'");
    ...
    $rss = mysqli_query($conn,"SELECT * FROM `db_courserec` WHERE `uid` = ".$_SESSION['ses_uid']);
    while($roww=mysqli_fetch_assoc($rss)){
        $reserved[] = $roww['cid'];
    }
...
// foreach($assets as $k => $v){
    ...
    <div class="card">
        <div class="media media-2x1 gd-primary">
            <a class="media-content" href="?mod=course-detail&id=<?php echo $k;?>" style="background-image:url(uploads/course/<?php echo $k;?>.jpg?1);<?php 
...
<?php
    ...
    if(mysqli_num_rows($rss)!=0){
        $roww = mysqli_fetch_assoc($rss);
        echo '<a href="action.php?mod=course-checkin&code='.$k.'&id='.$roww['id'].'" id="btn-new" class="btn btn-sm box-shadows btn-rounded gd-danger text-white">
        Check-out
    </a>';

    // }else{
    //     echo '<a href="action.php?mod=course-checkin&code='.$k.'" id="btn-new" class="btn btn-sm box-shadows btn-rounded gd-primary text-white">
    //     Check-in
    // </a>';
    }
?>
                ...
                <div class="col-6 text-right">
                    <a href="?mod=course-detail&id=<?php echo $k;?>"  class="btn btn-sm box-shadows btn-rounded gd-secondary text-white">Read more</a> 
                </div>
            </div>
        </div>
    </div>
                                    ...
                                    <tbody>
                                            ...
                                            while($row=mysqli_fetch_assoc($rs)){       
                                                ...
                                                echo '<tr class=" " data-id="'.$row['id'].'">
                                                        ...
                                                        <td>
                                                        <a href="?mod=course-reserve&id='.$row['id'].'"  class="btn btn-sm box-shadows btn-rounded gd-secondary text-white">Reserve</a>
