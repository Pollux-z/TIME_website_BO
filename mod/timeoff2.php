...
<div class="row">
    ...
    if($_SESSION['ses_ulevel']>=1){    
        $rss = mysqli_query($conn,"SELECT DISTINCT date FROM `db_timeoff_date` a join db_timeoff b WHERE b.ttype not like 'a' and a.date >= '$today' and a.tid = b.id and (b.status = 2 or b.status = 1) ORDER BY a.`date` ASC limit 5");
        while($roww=mysqli_fetch_assoc($rss)){
            $days[] = $roww['date'];
        }

        $sql = "SELECT date,time,ttype,uid FROM `db_timeoff_date` a join db_timeoff b where b.ttype not like 'a' and a.tid = b.id and (b.status = 2 OR b.status = 1) and a.date in ('".implode('\',\'',$days)."')";
...
<div class="table-responsive">
    ...
    // <a href="?mod='.$md.'-edit&id='.$row['id'].'" class="item-title text-color ">
     ...
     echo '                            </div>';
         ...
         }elseif(isset($teamlead)&&$row['status']!=2&&$row['status']!=3){
            echo '<div class="item-action dropdown">
                 <a href="#" data-toggle="dropdown" class="text-muted">
                     <i data-feather="more-vertical"></i>
                 </a>
