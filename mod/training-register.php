...
<div class="btn-group float-right">
    ...
    <a href="?mod=training-register" class="btn btn-outline-primary active">Registration Status</a>
    <a href="?mod=training-summary" class="btn btn-outline-primary">Summary Report</a>
</div>

<h3 class="mb-5">Registration Status</h3>
                                ...
                                <table id="datatable" class="table table-theme table-row v-middle" data-plugin="dataTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><span class="text-muted">Slip</span></th>
                                            <th></th>
                                        </tr>
                                    </thead>
                                    <tbody><?php
    ...
    $rs = mysqli_query($conn,"SELECT a.*,b.name,b.nick,b.position FROM `db_training_register` a join db_employee b WHERE a.uid = b.id and a.`status` > 0 ORDER BY a.`id` DESC");
    ...
    while($row=mysqli_fetch_assoc($rs)){
                                                echo '<tr class=" " data-id="'.$row['id'].'">
                                                        ...
                                                        <td><a href="?mod=training-view&rid='.$row['id'].'">'.$row['name'].'</a></td>
                                                        <td>
                                                        '.$row['position'].'
                                                        </td><td>'.$courses[$row['tid']].'</td>
                                                        <td>
                                                    ...
                                                    echo '</td><td>';
                                                    if($row['slip_file']!=''){
                                                        echo '<a href="uploads/training/'.$row['slip_file'].'" target="_blank">Slip</a>';
                                                    }
                                                    echo '</td>
                                                        <td>
                                                            <div class="item-action dropdown">
                                                                ...
                                                                Successful Payment
                                                                    ...
                                                                    <a class="dropdown-item edit" href="?mod=training-view&rid='.$row['id'].'">
                                                                        Edit
                                                                    </a>
                                                                </div>
                                                            </div>
