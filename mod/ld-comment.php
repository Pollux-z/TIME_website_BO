...
<div class="row" style="height:50px;">
    ...
    <div class="col-6">
        <div class="btn-group float-right">
            ...
            <a href="?mod=ld-comment" class="btn btn-outline-primary active">Comment</a>
...
<?php }else{
    ...
    $rs = mysqli_query($conn,"SELECT * FROM `db_ld_log_workshop` WHERE `status` = 1");
...
<div class="table-responsive">
    ...
    while($row=mysqli_fetch_assoc($rs)){
                    echo '<tr class=" " data-id="'.$row['id'].'">
                            ...
                            <td>'.$row['sid'].'</td>
                            <td class="flex"><a href="/uploads/ld/'.$row['attach'].'" target="_blank">'.$row['attach'].'</a></td>
                            <td><a href="?mod=ld-comment&id='.$row['id'].'">Comment</a> | <a href="action.php?mod=del&page=ld-comment&id='.$row['id'].'" class="text-danger" onclick="return confirm(\'Are you sure you want to delete this item?\');">Delete</a></td></tr>';
                }
            ?>                                        
